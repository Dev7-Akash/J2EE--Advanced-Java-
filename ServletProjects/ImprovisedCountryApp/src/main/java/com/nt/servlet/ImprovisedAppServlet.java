package com.nt.servlet;

import java.io.IOException;
import java.io.PrintWriter;
import java.util.Date;

import org.apache.naming.java.javaURLContextFactory;

import jakarta.servlet.ServletException;
import jakarta.servlet.http.HttpServlet;
import jakarta.servlet.http.HttpServletRequest;
import jakarta.servlet.http.HttpServletResponse;

public class ImprovisedAppServlet extends HttpServlet {
	
//Approch::1	
	/*
	 * @Override public void service(HttpServletRequest req, HttpServletResponse
	 * res)throws ServletException, IOException{
	 * 
	 * PrintWriter pw=res.getWriter();
	 * 
	 * res.setContentType("text/html");
	 * 
	 * int countryIndex=Integer.parseInt(req.getParameter("country"));
	 * 
	 * String countries[]=new String[] {"New Delhi",
	 * "Washington DC","CannBera","Bejing","Dhaka","Mosco"};
	 * 
	 * pw.
	 * println("<h1 style='color:red;text-align:center'>The Capital City Name Is::"
	 * +countries[countryIndex]+"</h1>");
	 * 
	 * pw.println("<h1 style='text-align:center'><a href='input.html'>Home</a></h1>"
	 * );
	 * 
	 * pw.close(); }//service
	 * 
	 * 
	 */
	
	
	//Approch::2==>version::1
	/*
	 * @Override public void doGet(HttpServletRequest req, HttpServletResponse
	 * res)throws ServletException, IOException{
	 * 
	 * System.out.println("ImprovisedAppServlet.doGet");
	 * 
	 * 
	 * PrintWriter pw=res.getWriter();
	 * 
	 * res.setContentType("text/html");
	 * 
	 * int countryIndex=Integer.parseInt(req.getParameter("country"));
	 * 
	 * String countries[]=new String[] {"New Delhi",
	 * "Washington DC","CannBera","Bejing","Dhaka","Mosco"};
	 * 
	 * pw.
	 * println("<h1 style='color:red;text-align:center'>The Capital City Name Is::"
	 * +countries[countryIndex]+"</h1>");
	 * 
	 * pw.println("<h1 style='text-align:center'><a href='input.html'>Home</a></h1>"
	 * );
	 * 
	 * pw.close(); }//dopost
	 * 
	 * public void doPost(HttpServletRequest req, HttpServletResponse res)throws
	 * ServletException, IOException{
	 * System.out.println("ImprovisedAppServlet.doPost");
	 * 
	 * doGet(req, res);
	 * 
	 * }
	 */

	
	//Approch::2==>version::2	

	
	/*
	 * @Override public void doPost(HttpServletRequest req, HttpServletResponse
	 * res)throws ServletException, IOException{
	 * 
	 * PrintWriter pw=res.getWriter();
	 * 
	 * res.setContentType("text/html");
	 * 
	 * int countryIndex=Integer.parseInt(req.getParameter("country"));
	 * 
	 * String countries[]=new String[] {"New Delhi",
	 * "Washington DC","CannBera","Bejing","Dhaka","Mosco"};
	 * 
	 * pw.
	 * println("<h1 style='color:red;text-align:center'>The Capital City Name Is::"
	 * +countries[countryIndex]+"</h1>");
	 * 
	 * pw.println("<h1 style='text-align:center'><a href='input.html'>Home</a></h1>"
	 * ); System.out.println("ImprovisedAppServlet.doPost"); pw.close();
	 * 
	 * }//dopost
	 * 
	 * public void doGet(HttpServletRequest req, HttpServletResponse res)throws
	 * ServletException, IOException{ PrintWriter pw=res.getWriter();
	 * 
	 * res.setContentType("text/html");
	 * 
	 * pw.
	 * println("<h1 style='color:orange;text-align:center'>System Date & Time Is::"
	 * +new java.util.Date()+"</h1>");
	 * 
	 * pw.println("<h1 style='text-align:center'><a href='input.html'>Home</a></h1>"
	 * ); System.out.println("ImprovisedAppServlet.doGet"); }
	 * 
	 * 
	 */
	
	
	//Approch::3==>version::1	
	
	
	/*
	 * public void process(HttpServletRequest req, HttpServletResponse res)throws
	 * ServletException, IOException{
	 * 
	 * PrintWriter pw=res.getWriter();
	 * 
	 * res.setContentType("text/html");
	 * 
	 * int countryIndex=Integer.parseInt(req.getParameter("country"));
	 * 
	 * String countries[]=new String[] {"New Delhi",
	 * "Washington DC","CannBera","Bejing","Dhaka","Mosco"};
	 * 
	 * pw.
	 * println("<h1 style='color:red;text-align:center'>The Capital City Name Is::"
	 * +countries[countryIndex]+"</h1>");
	 * 
	 * pw.println("<h1 style='text-align:center'><a href='input.html'>Home</a></h1>"
	 * ); pw.close(); }
	 * 
	 * public void doPost(HttpServletRequest req, HttpServletResponse res)throws
	 * ServletException, IOException{ process(req, res);
	 * 
	 * System.out.println("ImprovisedAppServlet.doPost");
	 * 
	 * 
	 * }
	 * 
	 * public void doGet(HttpServletRequest req, HttpServletResponse res)throws
	 * ServletException, IOException{ process(req, res);
	 * 
	 * System.out.println("ImprovisedAppServlet.doGet");
	 * 
	 * 
	 * }
	 * 
	 */
	
	//Approch::3==>version::2	

	
	
	public void processGet(HttpServletRequest req, HttpServletResponse res)throws ServletException, IOException{
		
		PrintWriter pw=res.getWriter();
		  
		  res.setContentType("text/html");
		  
		  pw.println("<h1 style='color:orange;text-align:center'>System Date & Time Is::"+new java.util.Date()+"</h1>");
		  
		  pw.println("<h1 style='text-align:center'><a href='input.html'>Home</a></h1>");
		  pw.close(); 

	}
		  
	
	
	public void processPost(HttpServletRequest req, HttpServletResponse res)throws ServletException, IOException{
		
		PrintWriter pw=res.getWriter();
		  
		  res.setContentType("text/html");
		  
		  int countryIndex=Integer.parseInt(req.getParameter("country"));
		  
		  String countries[]=new String[] {"New Delhi", "Washington DC","CannBera","Bejing","Dhaka","Mosco"};
		  
		  pw.
		  println("<h1 style='color:red;text-align:center'>The Capital City Name Is::"+countries[countryIndex]+"</h1>");
		  
		  pw.println("<h1 style='text-align:center'><a href='input.html'>Home</a></h1>"
		  );
		  pw.close(); 
	}
	
	public void doPost(HttpServletRequest req, HttpServletResponse res)throws ServletException, IOException{
		
		processPost(req, res);
		System.out.println("ImprovisedAppServlet.doPost");
		
	}

	public void doGet(HttpServletRequest req, HttpServletResponse res)throws ServletException, IOException{
	    processGet(req, res);
		
		System.out.println("ImprovisedAppServlet.doGet");
}
	
}
	
	

